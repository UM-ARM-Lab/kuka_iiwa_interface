...
public class LCMRobotInterface extends RoboticsAPIApplication implements LCMSubscriber
{
    ...
    public void initialize()
    {
        ...
        try
        {
            ...
            // Only initialize gripper if tool was successfully created
            getLogger().info("Initializing Gripper");
            gripper_ = new Robotiq3FingerGripper(iiwa7_arm_, getLogger());
            if (!gripper_.InitializeGripper()) {
                getLogger().error("Failed to initialize gripper. Falling back to flange mode.");
                tool_ = null;
        ...
        try
        {
            ...
            getLogger().info("destination: " + LCMURLs.DEFAULT_DEST_URL);
            ...
            getLogger().info("self: " + LCMURLs.DEFAULT_SELF_URL);
    ...
    private class Robotiq3FingerGripperPublisher
    {
        ...
        public Robotiq3FingerGripperPublisher()
        {
            ...
            if((byte) Robotiq3FingerGripperStatus.InitializationStatus.GRIPPER_RESET.ordinal() != robotiq_3finger_status.GRIPPER_RESET)
            {
                getLogger().warn("Gripper reset status mismatch");
            }
            if((byte) Robotiq3FingerGripperStatus.InitializationStatus.GRIPPER_ACTIVATION.ordinal() != robotiq_3finger_status.GRIPPER_ACTIVATION)
            {
                getLogger().warn("Gripper activation status mismatch");
            }       
            if((byte) Robotiq3FingerGripperStatus.GripperActionStatus.GRIPPER_STOPPED_OR_BUSY.ordinal() != robotiq_3finger_status.GRIPPER_STOPPED_OR_BUSY)
            {
                getLogger().warn("Gripper stopped or busy status mismatch");
            }
            if((byte) Robotiq3FingerGripperStatus.GripperActionStatus.GRIPPER_GOTO.ordinal() != robotiq_3finger_status.GRIPPER_GOTO)
            {
                getLogger().warn("Gripper goto status mismatch");
            }
            if((byte) Robotiq3FingerGripperStatus.GripperSystemStatus.GRIPPER_RESET_OR_AUTO_RELEASE.ordinal() != robotiq_3finger_status.GRIPPER_RESET_OR_AUTO_RELEASE)
            {
                getLogger().warn("Gripper reset or auto release status mismatch");
            }
            if((byte) Robotiq3FingerGripperStatus.GripperSystemStatus.GRIPPER_ACTIVATION_IN_PROGRESS.ordinal() != robotiq_3finger_status.GRIPPER_ACTIVATION_IN_PROGRESS)
            {
                getLogger().warn("Gripper activation in progress status mismatch");
            }
            if((byte) Robotiq3FingerGripperStatus.GripperSystemStatus.GRIPPER_MODE_CHANGE_IN_PROGESS.ordinal() != robotiq_3finger_status.GRIPPER_MODE_CHANGE_IN_PROGRESS)
            {
                getLogger().warn("Gripper mode change in progress status mismatch");
            }
            if((byte) Robotiq3FingerGripperStatus.GripperSystemStatus.GRIPPER_ACTIVATION_MODE_CHANGE_COMPLETE.ordinal() != robotiq_3finger_status.GRIPPER_ACTIVATION_MODE_CHANGE_COMPLETE)
            {
                getLogger().warn("Gripper activation mode change complete status mismatch");
            }
            if((byte) Robotiq3FingerGripperStatus.GripperMotionStatus.GRIPPER_STOPPED_UNKNOWN.ordinal() != robotiq_3finger_status.GRIPPER_STOPPED_UNKNOWN)
            {
                getLogger().warn("Gripper stopped unknown status mismatch");
            }
            if((byte) Robotiq3FingerGripperStatus.GripperMotionStatus.GRIPPER_IN_MOTION.ordinal() != robotiq_3finger_status.GRIPPER_IN_MOTION)
            {
                getLogger().warn("Gripper in motion status mismatch");
            }
            if((byte) Robotiq3FingerGripperStatus.GripperMotionStatus.GRIPPER_ONE_OR_TWO_STOPPED_EARLY.ordinal() != robotiq_3finger_status.GRIPPER_ONE_OR_TWO_STOPPED_EARLY)
            {
                getLogger().warn("Gripper one or two stopped early status mismatch");
            }
            if((byte) Robotiq3FingerGripperStatus.GripperMotionStatus.GRIPPER_ALL_STOPPED_EARLY.ordinal() != robotiq_3finger_status.GRIPPER_ALL_STOPPED_EARLY)
            {
                getLogger().warn("Gripper all stopped early status mismatch");
            }
            if((byte) Robotiq3FingerGripperStatus.GripperMotionStatus.GRIPPER_ALL_AT_REQUESTED.ordinal() != robotiq_3finger_status.GRIPPER_ALL_AT_REQUESTED)
            {
                getLogger().warn("Gripper all at requested status mismatch");
            }   
                ...
                public void run ()
                {
                    if (tool_ != null && gripper_ != null)
                    {
                        try {
                            ...
                            getLogger().error("Error publishing gripper status: " + e.getMessage());
                        }
                    }
                }
            };
        ...
        private void executeCommand(robotiq_3finger_command cmd)
        {
            if (tool_ != null && gripper_ != null)
            {
                try {
                    ...
                    getLogger().error("Error executing gripper command: " + e.getMessage());
                }
            } else {
                getLogger().warn("Gripper command received but no gripper is attached");
            }
        }
    }
}
