...
def DefaultMotionStatus(arm="right_arm"):
    """
...
def DefaultRobotiq3FingerStatus():
    """
...
class MinimalFakeArmInterface:
    def __init__(self,
        ...
        self.motion_status_msg =                  DefaultMotionStatus(arm=arm_name)
        self.gripper_status_msg =                 DefaultRobotiq3FingerStatus()
    ...
    def gripper_status_feedback_thread(self):
        r = rospy.Rate(gripper_feedback_rate)
        while not rospy.is_shutdown():
            with self.input_mtx:
