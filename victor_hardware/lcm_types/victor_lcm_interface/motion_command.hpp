...
#ifndef __victor_lcm_interface_motion_command_hpp__
#define __victor_lcm_interface_motion_command_hpp__
...
class motion_command {
 public:
  victor_lcm_interface::joint_value_quantity joint_position;

  victor_lcm_interface::joint_value_quantity joint_velocity;

  victor_lcm_interface::cartesian_pose cartesian_pose;
...
int motion_command::getEncodedSize() const { return 8 + _getEncodedSizeNoHash(); }
...
const char *motion_command::getTypeName() { return "motion_command"; }
...
uint64_t motion_command::_computeHash(const __lcm_hash_ptr *p) {
  ...
  for (fp = p; fp != NULL; fp = fp->parent)
    if (fp->v == motion_command::getHash) return 0;
  const __lcm_hash_ptr cp = {p, (void *)motion_command::getHash};

  uint64_t hash = 0xc0714d5769ac13acLL + victor_lcm_interface::joint_value_quantity::_computeHash(&cp) +
