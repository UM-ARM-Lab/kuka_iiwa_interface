...
int motion_status::getEncodedSize() const { return 8 + _getEncodedSizeNoHash(); }
...
const char *motion_status::getTypeName() { return "motion_status"; }
...
uint64_t motion_status::_computeHash(const __lcm_hash_ptr *p) {
  ...
  for (fp = p; fp != NULL; fp = fp->parent)
    if (fp->v == motion_status::getHash) return 0;
  const __lcm_hash_ptr cp = {p, (void *)motion_status::getHash};

  uint64_t hash = 0x8d25644d75239c66LL + victor_lcm_interface::joint_value_quantity::_computeHash(&cp) +
