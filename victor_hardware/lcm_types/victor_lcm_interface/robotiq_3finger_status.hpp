...
int robotiq_3finger_status::getEncodedSize() const { return 8 + _getEncodedSizeNoHash(); }
...
const char *robotiq_3finger_status::getTypeName() { return "robotiq_3finger_status"; }
...
uint64_t robotiq_3finger_status::_computeHash(const __lcm_hash_ptr *p) {
  ...
  for (fp = p; fp != NULL; fp = fp->parent)
    if (fp->v == robotiq_3finger_status::getHash) return 0;
  const __lcm_hash_ptr cp = {p, (void *)robotiq_3finger_status::getHash};

  uint64_t hash = 0x167393c65a518866LL + victor_lcm_interface::robotiq_3finger_actuator_status::_computeHash(&cp) +
