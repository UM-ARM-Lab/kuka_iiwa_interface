...
public class LCMRobotInterface extends RoboticsAPIApplication implements LCMSubscriber
{
	...
	private armlab.lcm.msgs.joint_path_execution_parameters joint_path_execution_params_;
	private armlab.lcm.msgs.cartesian_path_execution_parameters cartesian_path_execution_params_;
	...
	private class MotionStatusPublisher
	{
		...
		public MotionStatusPublisher()
		{
			...
			motion_status_msg_.measured_cartesian_pose_raw = Conversions.CVQInitializer(0.0);
			motion_status_msg_.commanded_cartesian_pose_raw = Conversions.CVQInitializer(0.0);
			...
			feedback_loop_task_ = new TimerTask()
			{
			    ...
			    public void run ()
				{
					synchronized (arm_io_lock_)
					{
						...
						Conversions.TransformationToCVQ(measured_world_ee, motion_status_msg_.measured_cartesian_pose_raw);
						Conversions.TransformationToCVQ(commanded_world_ee, motion_status_msg_.commanded_cartesian_pose_raw);
	...
	private class ControlModeCommandHandler
	{
		...
		public void handle(control_mode_parameters cmd)
		{
	...
	private class MotionCommandHandler
	{
		...
		public void handle(motion_command cmd)
		{
			switch (cmd.control_mode.mode)
			{
				case control_mode.JOINT_POSITION:
				{
					handleJointPositionCommand(cmd, ControlMode.JOINT_POSITION);
					break;
				}
				case control_mode.JOINT_IMPEDANCE:
				{
					handleJointPositionCommand(cmd, ControlMode.JOINT_IMPEDANCE);
					break;
				}
				case control_mode.CARTESIAN_POSE:
				{
					handleCartesianPoseCommand(cmd, ControlMode.CARTESIAN_POSE);
					break;
				}
				case control_mode.CARTESIAN_IMPEDANCE:
				{
					handleCartesianPoseCommand(cmd, ControlMode.CARTESIAN_IMPEDANCE);
					break;
				}
				default:
				{
		...
		private void handleJointPositionCommand(motion_command cmd, ControlMode control_mode)
		{
			synchronized (this)
			{
		...
		private void handleCartesianPoseCommand(motion_command cmd, ControlMode control_mode)
		{
			synchronized (this)
			{	
		...
		public JointPosition getJointPositionTarget()
		{
			synchronized (this)
			{
				if (control_mode_ != null && control_mode_ == ControlMode.JOINT_POSITION)
				{
					control_mode_ = null;
		...
		public Frame getCartesianPoseTarget()
		{
			synchronized (this)
			{
				if (control_mode_ != null && control_mode_ == ControlMode.CARTESIAN_POSE)
				{
					control_mode_ = null;
		...
		public JointPosition getJointImpedanceTarget()
		{
			synchronized (this)
			{
				if (control_mode_ != null && control_mode_ == ControlMode.JOINT_IMPEDANCE)
				{
					control_mode_ = null;
					return joint_position_target_;
				}
		...
		public Frame getCartesianImpedanceTarget()
		{
			synchronized (this)
			{
				if (control_mode_ != null && control_mode_ == ControlMode.CARTESIAN_IMPEDANCE)
				{
					control_mode_ = null;
					return cartesian_pose_target_;
				}
	...
	{
		private void handle(robotiq_3finger_command cmd)
		{
			if (tool_ != null)
			{
				synchronized(gripper_)
