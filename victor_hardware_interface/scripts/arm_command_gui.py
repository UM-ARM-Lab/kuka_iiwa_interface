...
import rospkg
import rospy
from victor_hardware_interface.msg import Robotiq3FingerCommand, MotionCommand, MotionStatus, Robotiq3FingerStatus, ControlModeCommand
...
class Arm():
    ...
    def __init__(self, parent_layout, arm_name, box_row):
        ...
        self.reset_arm_slider_to_current_value_button = self.init_button('Reset the Sliders',1,self.reset_arm_slider_to_current_value)
        ...
        self.reset_gripper_position_slider_to_current_value_button = self.init_button('Reset the Sliders',8,self.reset_gripper_position_slider_to_current_value)
    ...
    def reset_arm_slider_to_current_value(self):
        ...
        joint_1_degrees = min(max(-170+arm_joint_limit_margin,round(local_arm_status.measured_joint_position.joint_1*rad_to_deg)),170-arm_joint_limit_margin)
        self.joint_1_slider.setValue(joint_1_degrees)

        joint_2_degrees = min(max(-120+arm_joint_limit_margin,round(local_arm_status.measured_joint_position.joint_2*rad_to_deg)),120-arm_joint_limit_margin)
        self.joint_2_slider.setValue(joint_2_degrees)

        joint_3_degrees = min(max(-170+arm_joint_limit_margin,round(local_arm_status.measured_joint_position.joint_3*rad_to_deg)),170-arm_joint_limit_margin)
        self.joint_3_slider.setValue(joint_3_degrees)

        joint_4_degrees = min(max(-120+arm_joint_limit_margin,round(local_arm_status.measured_joint_position.joint_4*rad_to_deg)),120-arm_joint_limit_margin)
        self.joint_4_slider.setValue(joint_4_degrees)

        joint_5_degrees = min(max(-170+arm_joint_limit_margin,round(local_arm_status.measured_joint_position.joint_5*rad_to_deg)),170-arm_joint_limit_margin)
        self.joint_5_slider.setValue(joint_5_degrees)

        joint_6_degrees = min(max(-120+arm_joint_limit_margin,round(local_arm_status.measured_joint_position.joint_6*rad_to_deg)),120-arm_joint_limit_margin)
        self.joint_6_slider.setValue(joint_6_degrees)

        joint_7_degrees = min(max(-170+arm_joint_limit_margin,round(local_arm_status.measured_joint_position.joint_7*rad_to_deg)),170-arm_joint_limit_margin)
    ...
    def change_control_mode(self,control_mode):
        ...
        new_control_mode = ControlModeCommand()
        new_control_mode.control_mode = control_mode

        if(control_mode == 0): # JOINT_POSITION
