...
inline msgs::ControlModeParameters mergeControlModeParameters(const msgs::ControlModeParameters& active_control_mode,
                                                              const msgs::ControlModeParameters& new_control_mode);
...
MinimalArmWrapperInterface::MinimalArmWrapperInterface(
    ...
    const std::string& set_control_mode_service, const std::string& gripper_command_topic,
    const std::string& gripper_status_topic,
    // LCM channels
    const std::string& motion_command_channel, const std::string& motion_status_channel,
  ...
  const auto motion_status_callback_fn = [&](const msgs::MotionStatus& motion_status) {
    return motionStatusLCMCallback(motion_status);
  };
  const auto control_mode_status_callback_fn = [&](const msgs::ControlModeParameters& control_mode_status) {
  ...
  const auto gripper_status_callback_fn = [&](const msgs::Robotiq3FingerStatus& gripper_status) {
    return gripperStatusLCMCallback(gripper_status);
  };
  robotiq_ptr_ = std::unique_ptr<Robotiq3FingerHardwareInterface>(new Robotiq3FingerHardwareInterface(
      send_lcm_ptr_, recv_lcm_ptr_, gripper_command_channel, gripper_status_channel, gripper_status_callback_fn));

  // Set up ROS interfaces
  nh_.setCallbackQueue(&ros_callback_queue_);
  motion_status_pub_ = nh_.advertise<msgs::MotionStatus>(motion_status_topic, 1, false);
  control_mode_status_pub_ = nh_.advertise<msgs::ControlModeParameters>(control_mode_status_topic, 1, false);
  gripper_status_pub_ = nh_.advertise<msgs::Robotiq3FingerStatus>(gripper_status_topic, 1, false);
  motion_command_sub_ =
  ...
  get_control_mode_server_ =
      nh_.advertiseService(get_control_mode_service, &MinimalArmWrapperInterface::getControlModeCallback, this);
}
...
std::pair<bool, std::string> MinimalArmWrapperInterface::validateJointPathExecutionParams(
    const msgs::JointPathExecutionParameters& params) {
  bool valid = true;
  std::string message;
...
std::pair<bool, std::string> MinimalArmWrapperInterface::validateCartesianPathExecutionParams(
    const msgs::CartesianPathExecutionParameters& params) {
  bool valid = true;
  std::string message;
...
std::pair<bool, std::string> MinimalArmWrapperInterface::validateJointImpedanceParams(
    const msgs::JointImpedanceParameters& params) {
  bool valid = true;
  std::string message;
...
std::pair<bool, std::string> MinimalArmWrapperInterface::validateCartesianImpedanceParams(
    const msgs::CartesianImpedanceParameters& params) {
  bool valid = true;
  std::string message;
  ...
  // TODO: original values set by Calder were < 0.1 and > 300.0 - documentation states < 0.0 and > 300.0; why was it set
  // to 0.1?
...
std::pair<bool, std::string> MinimalArmWrapperInterface::validateCartesianControlModeLimits(
    const msgs::CartesianControlModeLimits& params) {
  bool valid = true;
  std::string message;
...
std::pair<bool, std::string> MinimalArmWrapperInterface::validateControlMode(
    const msgs::ControlModeParameters& params) {
  bool valid = true;
  std::string message;
...
bool MinimalArmWrapperInterface::setControlModeCallback(msgs::SetControlMode::Request& req,
                                                        msgs::SetControlMode::Response& res) {
  Maybe::Maybe<msgs::ControlModeParameters> local_active_control_mode_copy;
  {
...
bool MinimalArmWrapperInterface::getControlModeCallback(msgs::GetControlMode::Request& req,
                                                        msgs::GetControlMode::Response& res) {
  UNUSED(req);
  std::lock_guard<std::mutex> lock(control_mode_status_mutex_);
...
std::pair<bool, std::string> MinimalArmWrapperInterface::validateJointPositions(
    const msgs::JointValueQuantity& positions) {
  // TODO: The function is not implemented yet; it should check that the commanded joint positions are within the joint
  // limits of the arm
  UNUSED(positions);
  return std::make_pair(true, std::string(""));
}

std::pair<bool, std::string> MinimalArmWrapperInterface::validateCartesianPose(const geometry_msgs::Pose& pose,
  ...
  if (frame != cartesian_control_frame_) {
    valid = false;
    message +=
        "+Commanded cartesian pose has the wrong frame, " + frame + " given, " + cartesian_control_frame_ + " expected";
  }
...
std::pair<bool, std::string> MinimalArmWrapperInterface::validateMotionCommand(
    const msgs::MotionCommand& command) const {
  Maybe::Maybe<msgs::ControlModeParameters> active_control_mode_cached;
  {
    std::lock_guard<std::mutex> lock(control_mode_status_mutex_);
  ...
  if (active_control_mode_cached.Valid()) {
    ...
    switch (command_motion_mode) {
      case msgs::ControlMode::JOINT_POSITION:
      case msgs::ControlMode::JOINT_IMPEDANCE:
        return validateJointPositions(command.joint_position);

      case msgs::ControlMode::CARTESIAN_POSE:
      case msgs::ControlMode::CARTESIAN_IMPEDANCE:
 ...
 */
void MinimalArmWrapperInterface::motionCommandROSCallback(const msgs::MotionCommand& command) {
  ...
  if (validity_check_results.first) {
    ...
    ROS_ERROR_STREAM_NAMED(ros::this_node::getName(),
                           "Arm motion command failed validity checks: " << validity_check_results.second);
  }
}
 ...
 */
void MinimalArmWrapperInterface::motionStatusLCMCallback(const msgs::MotionStatus& motion_status) {
...
std::pair<bool, std::string> MinimalArmWrapperInterface::validateFingerCommand(
    const msgs::Robotiq3FingerActuatorCommand& command) {
  bool valid = true;
  std::string message;
...
std::pair<bool, std::string> MinimalArmWrapperInterface::validateGripperCommand(
    const msgs::Robotiq3FingerCommand& command) {
 ...
 */
void MinimalArmWrapperInterface::gripperCommandROSCallback(const msgs::Robotiq3FingerCommand& command) {
  ...
  if (validity_check_results.first) {
    ...
    ROS_WARN_STREAM_NAMED(ros::this_node::getName(),
                          "Gripper command failed validity checks: " << validity_check_results.second);
  }
}
 ...
 * on the correct ROS topic
 */
void MinimalArmWrapperInterface::gripperStatusLCMCallback(const msgs::Robotiq3FingerStatus& gripper_status) {
...
inline bool jointPathExecutionParamsIsDefault(const msgs::JointPathExecutionParameters& params) {
  return (params.joint_relative_velocity == 0 && params.joint_relative_acceleration == 0 &&
          params.override_joint_acceleration == 0);
}

inline bool cartesianPathExecutionParamsIsDefault(const msgs::CartesianPathExecutionParameters& params) {
...
inline bool jointImpedanceParamsIsDefault(const msgs::JointImpedanceParameters& params) {
...
inline bool cartesianImpedanceParamsIsDefault(const msgs::CartesianImpedanceParameters& params) {
...
inline bool cartesianControlModeLimitsIsDefault(const msgs::CartesianControlModeLimits& params) {
...
inline msgs::ControlModeParameters mergeControlModeParameters(const msgs::ControlModeParameters& active_control_mode,
  ...
  ***************************************************************************************************************/
  ...
  msgs::ControlModeParameters merged_control_mode;
  // Copy the old over
  ...
  switch (new_control_mode.control_mode.mode) {
    case msgs::ControlMode::JOINT_IMPEDANCE:
      ...
      if (!cartesianImpedanceParamsIsDefault(new_control_mode.cartesian_impedance_params)) {
        ROS_WARN_NAMED(ros::this_node::getName(),
                       "The cartesian impedance parameters are specified but ignored in JOINT_IMPEDANCE mode.");
      }
      if (!cartesianControlModeLimitsIsDefault(new_control_mode.cartesian_control_mode_limits)) {
        ROS_WARN_NAMED(ros::this_node::getName(),
                       "The cartesian control mode limits are specified but ignored in JOINT_IMPEDANCE mode.");
      }
    ...
    case msgs::ControlMode::CARTESIAN_IMPEDANCE:
      ...
      if (!jointImpedanceParamsIsDefault(new_control_mode.joint_impedance_params)) {
        ROS_WARN_NAMED(ros::this_node::getName(),
                       "The joint impedance parameters are specified but ignored in CASRTESIAN_IMPEDANCE mode.");
      }
    ...
    case msgs::ControlMode::JOINT_POSITION:
      ...
      if (!jointImpedanceParamsIsDefault(new_control_mode.joint_impedance_params)) {
        ROS_WARN_NAMED(ros::this_node::getName(),
                       "The joint impedance parameters are specified but ignored in JOINT_POSITION mode.");
      }
      if (!cartesianImpedanceParamsIsDefault(new_control_mode.cartesian_impedance_params)) {
        ROS_WARN_NAMED(ros::this_node::getName(),
                       "The cartesian impedance parameters are specified but ignored in JOINT_POSITION mode.");
      }
      if (!cartesianControlModeLimitsIsDefault(new_control_mode.cartesian_control_mode_limits)) {
        ROS_WARN_NAMED(ros::this_node::getName(),
                       "The cartesian control mode limits are specified but ignored in JOINT_POSITION mode.");
      }
      if (!cartesianPathExecutionParamsIsDefault(new_control_mode.cartesian_path_execution_params)) {
        ROS_WARN_NAMED(ros::this_node::getName(),
    ...
    case msgs::ControlMode::CARTESIAN_POSE:
      ...
      if (!jointPathExecutionParamsIsDefault(new_control_mode.joint_path_execution_params)) {
        ROS_WARN_NAMED(ros::this_node::getName(),
                       "The joint path execution parameters are specified but ignored in CARTESIAN_POSE mode.");
      }
      if (!jointImpedanceParamsIsDefault(new_control_mode.joint_impedance_params)) {
        ROS_WARN_NAMED(ros::this_node::getName(),
                       "The joint impedance parameters are specified but ignored in CARTESIAN_POSE mode.");
      }
      if (!cartesianImpedanceParamsIsDefault(new_control_mode.cartesian_impedance_params)) {
        ROS_WARN_NAMED(ros::this_node::getName(),
                       "The cartesian impedance parameters are specified but ignored in CARTESIAN_POSE mode.");
      }
      if (!cartesianControlModeLimitsIsDefault(new_control_mode.cartesian_control_mode_limits)) {
        ROS_WARN_NAMED(ros::this_node::getName(),
